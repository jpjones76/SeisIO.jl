...
println("...IRISws...")
SAC = irisws(net="CC", sta="TIMB", cha="EHZ", s=ts, t=te, fmt="sacbl",w=true)
T = irisws(net="CC", sta="TIMB", cha="EHZ", s=ts, t=te, fmt="miniseed",w=true)
@test_approx_eq(SAC["data"][1], T.x[1][1])
@test_approx_eq(SAC["npts"], length(T.x[1]))

println("...IRISget...")
U = IRISget(chans, t=-300, w=true)
