...
function check_get_data!(S::SeisData, protocol::String, channels::Union{String, Array{String,1}};
    incr::DatePeriod=Day(1),
    max_retries::Integer=7,
...
reg::Array{Float64,1} = KW.reg                              ,  # Query region
  ...
  while i ≤ max_retries
    i += 1
    get_data!(S, protocol, channels, autoname=autoname, demean=demean, detrend=detrend, fmt=fmt, msr=msr, nd=nd, opts=opts, prune=prune, rad=rad, reg=reg, rr=rr, s=s, si=si, src=src, t=t, taper=taper, to=to, ungap=ungap, unscale=unscale, v=v, w=w, xf=xf, y=y)
    if has_data(S)
      if i > 1
...
reg::Array{Float64,1} = KW.reg                              ,  # Query region
  ...
  S = SeisData()
  check_get_data!(S, protocol, channels, incr=incr, max_retries=max_retries, autoname=autoname, demean=demean, detrend=detrend, fmt=fmt, msr=msr, nd=nd, opts=opts, prune=prune, rad=rad, reg=reg, rr=rr, s=s, si=si, src=src, t=t, taper=taper, to=to, ungap=ungap, unscale=unscale, v=v, w=w, xf=xf, y=y)
  return S
end
