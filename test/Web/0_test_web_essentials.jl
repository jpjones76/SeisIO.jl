...
(req, parsable) = get_HTTP_req(url, req_info_str, to, err=false)
@test typeof(req) == Array{UInt8,1}
@test startswith(String(req), "HTTP.Messages.Response")
@test parsable == false

(req,parsable) = get_HTTP_req(url, req_info_str, to, err=true)
@test typeof(req) == Array{UInt8,1}
@test startswith(String(req), "HTTP.Messages.Response")
@test parsable == false
...
(req,parsable) = get_HTTP_req(url, req_info_str, to, err=true)
@test typeof(req) == Array{UInt8,1}
@test Array{UInt8,1}("HTTP/1.1 400 Bad Request") == req[1:24]
