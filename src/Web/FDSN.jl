...
function FDSNget(C::Array{String,2};
  ...
  for j = 1:size(C,1)
    ...
    if R.status == 200
      w && savereq(R.body, "mseed", C[j,1], C[j,2], C[j,3], C[j,4], d0, d1, string(q))
      S = parsemseed(IOBuffer(R.body), false, v)
...
FDSNget(S::String;
  ...
  y=false::Bool,
  si=true::Bool,
  to=30::Int64) = FDSNget(parse_chstr(S, fdsn=true), src=src, q=q, s=s, t=t, v=v, w=w, y=y, si=si, to=to)
...
FDSNget(S::Array{String,1};
  ...
  y=false::Bool,
  si=true::Bool,
  to=30::Int64) = FDSNget(parse_charr(S, fdsn=true), src=src, q=q, s=s, t=t, v=v, w=w, y=y, si=si, to=to)
...
function FDSNevq(ts::String;
  ...
  for k in sources
      ...
      R = request("GET", url, webhdr(), readtimeout=to)     #R = get(url, timeout=to, headers=webhdr())
      if R.status == 200
...
function FDSNevt(evt::String, cc::String;
  ...
  pstr = Array{String,1}(undef, S.data.n)
  bads = falses(S.data.n)
  for i = 1:S.data.n
    pdat = get_pha(S.data.misc[i]["dist"], S.hdr.loc[3], to=to, v=v)
    if pha == "all"
      j = get_phase_start(pdat)
      s = Float64(Meta.parse(pdat[j,4])) - spad
      t = 2.0 * Float64(Meta.parse(pdat[get_phase_end(pdat),4]))
