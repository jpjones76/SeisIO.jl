...
"""
function batch_read(files::Array{ByteString,1}; ftype=ftype::ASCIIString, fs=0.0::Float64)
  file0 = files[1]
  files = files[2:end]
  NF = length(files)
  ...
  DT = collect(repeated(round(Int, dt*1.0e6),NF))
  ts = Array{Int64,1}(NF)
  nx = Array{Int64,1}(NF)
  rr = Array{Float64,1}(NF)
  gain = Array{Float32,1}(NF)

  if ftype == "SAC"
    fmt = collect(repeated(Float32, NF))
    for i = 1:NF
...
function segy_geth(fname)
  ...
  npts = read(f, Int32)
  close(f)
  nx = vals[7] == 32767 ? npts : vals[7]                                  # nx
  gain = (scale_fac > 0 && vals[9] > 0) ? (scale_fac / vals[9]) : 1.0     # gain
  ...
  return (t, nx, fmt, gain, dt)
end
# =============================================================================

function read_bat_1(fname, nx, rr, i, fmt, gain, ts, dt, os, tt, xx)
  f = open(fname,"r")
  seek(f, os)
  x = read(f, fmt, nx).*gain
  close(f)
  if !isapprox(rr, 1.0)
    x = resample(x, rr)
  end
  lx = length(x)
  xx[i+1:i+lx] = x
  tt[i+1:i+lx] = ts + cumsum(collect(repeated(dt, lx)))
  return
end

# =============================================================================
function sac_geth(fname)
  f = open(fname, "r")
  fv = read(f, Float32, 70)
  iv = read(f, Int32, 16)
  g = fv[4] == -12345.0f0 ? 1.0 : fv[4]
  (m,d) = j2md(iv[1],iv[2])
  t = round(Int, d2u(DateTime(iv[1],m,d,iv[3],iv[4],iv[5]))*1.0e6
    + (iv[6]*1000) + (fv[6] == -12345.0 ? 0.0 : fv[6])*1000)
  close(f)
  return (t, iv[10], g, fv[1])
end
