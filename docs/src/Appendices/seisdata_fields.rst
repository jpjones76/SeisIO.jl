...
.. [#] Arrays in ``:misc`` should contain a single variablle type (e.g. ``Array{Float64,1}``, never ``Array{Any,1}``). See the :ref:`SeisIO file format description <seisdata_misc_types>` for a full list of allowed value types in ``:misc``.
...
For *irregularly sampled data* (``fs = 0``), ``t`` is a dense delta-compressed representation of *time stamps for each sample*.
